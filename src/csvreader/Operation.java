	...
	public static BLEModel generateBLEModel(ArrayList<Observation> olist){
		   ...
		   for(int i = 0;i<blelist.size();i++){
			   String key = blelist.get(i).uuid+"_"+blelist.get(i).major+"_"+blelist.get(i).minor;
	        	...
	        	if(bm.avemap.get(key) != null){
		        	...
		        	double prob = 0.0;
		        	//受信電波強度の確率密度
		        	//prob_dens = (1/((Math.sqrt(2*Math.PI))*std))*Math.exp(-(Math.pow(blelist.get(i).rssi-ave, 2)/(2*Math.pow(std, 2))));
		        	...
		        	//NormalDistribution nd = new NormalDistribution(ave,Math.pow(std, 2));
		        	//prob_dens = nd.frequencyOf(blelist.get(i).rssi);
		        	//System.out.println(prob_dens);
		        	//平均の電波強度の確率密度
		        	//ave_dens = (1/((Math.sqrt(2*Math.PI))*std))*Math.exp(-(Math.pow(ave-ave, 2)/(2*Math.pow(std, 2))));
	   ...
	   public static double calcProbability(ArrayList<WifiObservation> wifilist, WifiRoomModel wm,String roomid){
		   ...
		   for(int i = 0;i<wifilist.size();i++){
	        	...
	        	if(wm.avemap.get(wifilist.get(i).bssid) != null){
		        	...
		        	double ave_dens = 0.0;
		        	double prob = 0.0;
		        	prob_dens = (1/((Math.sqrt(2*Math.PI))*std))*Math.exp(-(Math.pow(wifilist.get(i).rssi-ave, 2)/(2*Math.pow(std, 2))));
		        	ave_dens = (1/((Math.sqrt(2*Math.PI))*std))*Math.exp(-(Math.pow(ave-ave, 2)/(2*Math.pow(std, 2))));
		        	prob = (prob_dens/ave_dens);
	   ...
	   public static void print(TreeMap<String,BLEModel> blemap,TreeMap<String,WifiRoomModel> wifimap ){
		   ...
		   while(itb.hasNext()){
	        	...
	        	while(it.hasNext()){
	        		String key = it.next();
	        		System.out.println(key+",確率:"+bm.map.get(key)+",平均:"+bm.avemap.get(key)+",標準偏差:"+bm.stdmap.get(key));
	        	}
		   }
		   ...
		   while(itw.hasNext()){
	        	...
	        	while(ite.hasNext()){
	        		...
	        		String key = ite.next();
	        		System.out.println(key+",確率:"+wm.map.get(key)+",平均:"+wm.avemap.get(key)+",標準偏差:"+wm.stdmap.get(key));
	        	}
		   }
		  ...
		  /* Iterator<String> itw = wifimap.keySet().iterator();
		   while(itw.hasNext()){
	        	String roomid=itw.next();
	        	System.out.println(roomid);
	        	WifiModel wm = wifimap.get(roomid);
	        	Iterator<String> it = wm.map.keySet().iterator();
	        	while(it.hasNext()){
	        		String key = it.next();
	        		System.out.println(key+","+wm.map.get(key)+","+wm.avemap.get(key)+","+wm.stdmap.get(key));
	        	}
	        }*/
	   }
